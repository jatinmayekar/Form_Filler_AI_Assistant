...
if 'st_page_number' not in st.session_state:
    st.session_state['st_page_number'] = 0
...
def get_latest_unfilled_page_number():
    ...
    # Start from the latest filled page number
    for i in range(page_number, total_no_of_pages):
        flag = False
        with SuppressPrint():
...
def get_page_context():
    return (f"Provide a brief summary of the context for form fields on page {page_number}, focusing on key "
            f"details and instructions that clarify the information being requested. This summary should enable "
            f"understanding of the form's overall purpose and guide accurate field completion.")
...
def set_fields():
    ...
    # update input path pdf
    input_pdf_path = st.session_state['st_input_pdf_path'] = output_pdf_path

    # increment the page number to point to latest unfilled page
    page_number = st.session_state['st_page_number'] = page_number + 1
    return output_pdf_path
...
#   file=open("Parthshastra_For Print_V2_editable_OG.pdf", "rb"),
#   purpose='assistants'
# )
...
if 'assistant' not in st.session_state:
    st.session_state.assistant = st.session_state.client.beta.assistants.create(
        ...
        The PDF form path is always and must be provided by the user. You will be penalized if you use the uploaded pdf
        as input pdf. The uploaded pdf must only be used for reference/context/assistance."
        ...
        This is always provided by the user. Must not use the uploaded pdf as the input pdf.
        The uploaded pdf must only be used for reference/context/assistance.
        ...
        tools=[
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Set the current page number. In cases, where the page returns no fields or no "
                                   "empty fields, use this to move to the next page number",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "input_page_number": {
            ...
            #         "description": "Get the context/summary of the current page.",
            #         "parameters": {}
            #     }
            # },
            {
        ...
        while True:
            ...
            if run.status == "requires_action":
                ...
                for i in range(len(tool_calls)):
                    ...
                    # elif tool_calls[i].function.name == "get_page_context":
                    #     msg.append({
                    #         "tool_call_id": tool_calls[i].id,
                    #         "output": f"Context/Summary for the current page: {get_page_context()}"
                    #     })
